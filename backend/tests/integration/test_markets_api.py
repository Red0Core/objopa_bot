...
def mock_offers() -> list[Offer]:
    ...
    return [
        Offer(price=95.5, nickname="User1", finish_num=100, is_va=True, is_ba=False, 
              payment_types=["Тинек", "Сбер"], min_amount=1000, max_amount=30000, available_amount=500),
        Offer(price=96.0, nickname="User2", finish_num=200, is_va=False, is_ba=True, 
              payment_types=["Райф", "СБП"], min_amount=25000, max_amount=80000, available_amount=1000)
    ]
...
@pytest.mark.integration
@patch('backend.services.markets.bybit_p2p.get_p2p_orders')
def test_p2p_endpoint_service_error(
    mock_p2p_orders,
    client: TestClient
):
...
def mock_cbr_response():
    ...
    return {
        "USD": {"rate": 92.5, "diff": 0.5},
        "EUR": {"rate": 99.8, "diff": 0.3}
    }
...
def test_cbr_endpoint_success(
    ...
    mock_generate_html.return_value = "Курсы валют ЦБ РФ на сегодня:\n💵 Доллар США: 92.5 ₽ (+0.5)\n💶 Евро: 99.8 ₽ (+0.3)"
...
def test_exchanges_endpoint_success(
    mock_get_price,
    client: TestClient,
...
@pytest.mark.integration
@patch('backend.services.markets.exchanges.get_price_from_exchanges')
def test_exchanges_endpoint_error(
    mock_get_price,
    client: TestClient
):
    """Test exchange price endpoint when exchanges are unavailable"""
    # Setup mock to return error
    mock_get_price.return_value = {"symbol": None, "price": None, "error": "Symbol not found or exchanges unavailable"}
