...
class InstagramWebDownloader:
    ...
    async def _load_post(self, session: InstagramHttpSession, shortcode: str, canonical_url: str) -> InstagramPost:
        ...
        # Embed fallback for JSON metadata if we have zero media extracted
        try:
            embed_html = await self._load_html(session, f"https://www.instagram.com/p/{shortcode}/embed/captioned/")
            embed_candidates = self._extract_json_candidates(embed_html)
            for candidate in embed_candidates:
                post = self._safe_post_from_json(candidate, shortcode, canonical_url)
                if post.media:
                    return post

            # Try raw extraction on embed page as well
            raw_embed_post = self._post_from_raw_video_urls(embed_html, shortcode, canonical_url)
            if raw_embed_post and raw_embed_post.media:
                return raw_embed_post
        except Exception as e:
            logger.debug(f"Embed fallback failed for {shortcode}: {e}")
    ...
    def _pick_image_url(self, node: dict[str, Any]) -> str | None:
        ...
        # Handle fallback for display_url when no candidates exist
        if "display_url" in node and isinstance(node["display_url"], str):
             return html.unescape(node["display_url"])
    ...
    def _post_from_raw_video_urls(self, html_text: str, shortcode: str, canonical_url: str) -> InstagramPost:
        ...
        if not media_items:
            ...
            for pattern in image_patterns:
                for match in re.finditer(pattern, html_text, flags=re.IGNORECASE):
                    raw_url = match.group(1) if match.groups() else match.group(0)
                    image_url = self._decode_media_url(raw_url)
                    if self._looks_like_media_url(image_url) and "static.cdninstagram.com" not in image_url:
                        if "150x150" not in image_url:
                            media_items.append(InstagramMedia(url=image_url, is_video=False, index=len(media_items) + 1))
