    ...
    while True:
        ...
        for key in keys:
            ...
            try:
                ...
                relative_paths: list[str] = task_data["data"][
                    "relative_paths"
                ]  # Может быть список ссылок на изображения или файлы

                if len(f"select_image:{task_id}:1".encode("utf-8")) > 64:
                    logger.warning(
                        f"Callback data слишком длинная: {len(f'select_image:{task_id}:1'.encode('utf-8'))} байт"
                    )
                ...
                regenerate_button = InlineKeyboardButton(
                    text="🔄 Пересоздать", callback_data=f"select_image:{task_id}:-1"
                )
                ...
                media = [
                    InputMediaPhoto(media=FSInputFile(UPLOAD_DIR.joinpath(relative_path)))
                ...
                for retry in range(4, 8):
                    try:
                        ...
                        logger.info(
                            f"Отправлено сообщение с изображениями и кнопками за {time.time() - start} к {user_id}."
                        )
                        break
