...
async def get_price_handler(message: Message):
    ...
    async with AsyncClient() as session:
        try:
            ...
            logger.error(f"Ошибка получения цены: {traceback.format_exc()}")
...
async def get_cmc_handler(message: Message):
    ...
    logger.info(f"Успешно отправлен coinmarketcap {args[1]} к {message.from_user.id if message.from_user else 'unknown'}")
...
async def add_to_whitelist_coinmarketcap_handler(message: Message):
    ...
    try:
        async with AsyncClient() as session:
            response = await session.post(f"{BACKEND_ROUTE}/markets/crypto/whitelist", json={"symbol": symbol, "name": name})
            response.raise_for_status()
