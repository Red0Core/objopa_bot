...
async def get_forex_rub_rates_handler(message: Message):
    ...
    try:
        async with httpx.AsyncClient() as session:
            ...
            for data in (await arr):
                ...
                market = data['quote']
                today = data['rate']
                change_1d = (data['changes']['day1']['absolute'], data['changes']['day1']['percent'])
                change_7d = (data['changes']['day7']['absolute'], data['changes']['day7']['percent']) if data['changes']['day7'] else None
                change_30d = (data['changes']['day30']['absolute'], data['changes']['day30']['percent']) if data['changes']['day30'] else None
                # Формируем сообщение
                output += (
    ...
    logger.info(f"Успешно отправил рубль для {message.from_user.id if message.from_user else 'unknown'}")