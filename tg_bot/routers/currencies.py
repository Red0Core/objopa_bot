...
async def convert_currency_handler(message: Message):
    ...
    try:
        async with httpx.AsyncClient() as session:
            ...
            output += f"<code>{amount:.2f}</code> <b>{from_currency}</b> = <code>{result:.2f}</code> <b>{to_currency}</b>\n\n"
...
async def get_forex_rub_rates_handler(message: Message):
    ...
    try:
        async with httpx.AsyncClient() as session:
            ...
            forex_results = await asyncio.gather(
                fetch_currency_data("USD", "RUB"), 
                fetch_currency_data("EUR", "RUB")
            )
    ...
    await message.reply(output, parse_mode="html")
    logger.info(
        f"Успешно отправил курсы USD/EUR для {message.from_user.id if message.from_user else 'unknown'}"
    )
