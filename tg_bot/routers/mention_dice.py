...
async def handle_mention(message: Message, bot: Bot):
    ...
    system_prompt = """Ты — креативный и весёлый помощник. 
...
Если пользователь хочет узнать делать какое-то действие или нет, то:
- Если кубик показал 1-3: Напиши вдохновляющее письмо, объясняющее, почему это отличная идея и ты его поддерживаешь.
- Если кубик показал 4-6: Напиши креативный текст с элементами юмора, объясняющий, почему это плохая идея и ты его отговариваешь
Если пользователь имеет выбор между чем-то и чем-то, то используй результат кубика для принятия решения и к нему ответ:
- Если выборов несколько то, ты делаешь выбор в зависимости от результата кубика.
- Если выборов два, то:
  - Если кубик показал 1-3: Поддержи первый вариант.
  - Если кубик показал 4-6: Поддержи второй вариант.
"""
