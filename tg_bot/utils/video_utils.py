...
class VideoProcessor:
    ...
    async def convert_to_faststart(
        ...
        try:
            ...
            cmd.extend([
                ...
                "-movflags", "faststart",  # Включаем faststart
    ...
    async def optimize_video_for_telegram(
        ...
        try:
            ...
            logger.info(f"Analyzing {video_path.name}: {video_info.size_mb:.1f}MB, "
            ...
            if needs_compression:
                ...
                target_bitrate_kbps = await self._calculate_target_bitrate(
                    video_info, max_size_mb
                )
                ...
                cmd.extend([
                    ...
                    "-maxrate", f"{int(target_bitrate_kbps * self.config.max_bitrate_factor)}k",
                    "-bufsize", f"{int(target_bitrate_kbps * self.config.buffer_size_factor)}k",
                    "-c:a", "aac",
                    "-b:a", "128k"
                ])
