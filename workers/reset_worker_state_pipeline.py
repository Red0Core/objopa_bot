...
class ResetWorkerStatePipeline(BasePipeline):
    ...
    async def run(self) -> None:
        logger.info(f"ResetWorkerStatePipeline: Запуск для worker_id: {self.worker_id} (Task: {self.task_id})")
        ...
        try:
            ...
            logger.success(f"Worker {self.worker_id}: Состояние успешно сброшено (Task: {self.task_id}).")
            if self.user_id:
            ...
            logger.error(f"Worker {self.worker_id}: Ошибка при сбросе состояния: {e}", exc_info=True)
            if self.user_id:
                await send_notification(
