    ...
    for attempt in range(max_retries):
        try:
            ...
            with open(file_path, "rb") as file_data:
                ...
                async with AsyncClient(timeout=60.0) as client:
                    ...
                    logger.info(f"Файл {file_name} успешно загружен сек (попытка {attempt + 1}). Backend: {filename_on_backend}")
        ...
        # --- Обработка ошибок, при которых стоит повторить ---
            ...
            if 500 <= e.response.status_code < 600:
                logger.warning(f"Попытка {attempt + 1}/{max_retries} не удалась (ошибка сервера {e.response.status_code}): {e.response.text}")
            else:
                # Клиентские ошибки (4xx) или другие HTTP ошибки - не повторяем
                logger.error(f"Ошибка HTTP {e.response.status_code} при загрузке файла (не будет повторена): {e.response.text}")
        ...
        # --- Обработка других ошибок, которые не повторяем ---
            ...
            logger.exception(f"Непредвиденная ошибка при загрузке файла на попытке {attempt + 1} (не будет повторена): {e}")
